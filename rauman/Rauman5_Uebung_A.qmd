...
names(sheets1) <- sheets1

header_above <- rep(3,length(sheets1))
names(header_above) <- sheets1

sheets1_list <- imap(sheets1, function(x,y){
  read_ods(reclassify_ods, x)
})
 
sheets1_cbind <- do.call(cbind, sheets1_list) 

sheets1_cbind|>
  kableExtra::kbl(col.names = stringr::str_remove(colnames(sheets1_cbind), "\\w+\\.")) |>
  kable_classic()  |>
...
```{r}
# library(ggplot2)
# imap_dfr(sheets1_list, ~mutate(.x, var = .y)) |>
#   mutate(across(c(Von,Bis, Bewertung),as.numeric)) |>
#   ggplot(aes(Bis, Bewertung, colour = var, group = var)) +
#   geom_point() +
#   geom_line()
```
...
wald_klassen <- as.matrix(wald_klassen)  
...
```{r}
# Siedlungsgebiet
siedlungsgebiet_klassen <- tribble(
  ~von, ~bis, ~zu,
    0,   500, 0.0,
  500,  1000, 0.2,
  1000, 1500, 0.4,
