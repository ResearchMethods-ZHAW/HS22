...
Das Geopackage beinhaltet 3 Layers (siehe st_layers("zweitwohnungsinitiative.gpkg")). In jedem Layer sind die Abstimmungsresultate auf eine andere politische Ebene aggregiert. Wir started mit der Aggregationsstufe "kanton"
...
Dafür erstellen wir als erstes die Variabeln `y` für $y$ und `ybar` für $\bar{y}$:
```{r}
#| echo: true
# Die Werte aller Kantone in einer Variabel abspeichern
y <- zweitwohnung_kanton$ja_in_percent

# den Mittelwert über alle Kantone berechnen
ybar <- mean(y, na.rm = TRUE)
```

Bevor wir den *For Loop* ausführen können, müssen wir ein Vektor der Länge 26 (Anzahl Kantone) erstellen um die Resultate zu speichern (hier: `result`).
...
Dies lässt sich folgendermassen berechnen:
...
$$I = \frac{n}{\sum_{i=1}^n (y_i - \bar{y})^2} \times \frac{{\color{red}\sum_{i=1}^n \sum_{j=1}^n} w_{ij}(y_i - \bar{y})(y_j - \bar{y})}{{\color{orange}\sum_{i=1}^n \sum_{j=1}^n} w_{ij}}$$

Für die Berechnung von `zaehler2` haben wir ein doppeltes Summenzeichen, die wir auflösen müssen. Das ist an sich schon Komplex genug, wesshalb wir den Term $w_{ij}$ an dieser Stelle mal ignorieren.

$\sum_{i=1}^n \sum_{j=1}^n$ kann man wie zwei verschachtelte *For loops* sehen. Der äussere *For loop* iteriert von `1` bis `n` mit der Variabel `i`. Der innere *For loop* iteriert von `1` bis `n` mit der Variabel `j`. 

```{r}
#| echo: true

resultat_aussen <- numeric(26)

for (i in 1:n){
  resultat_innen <- numeric(26)
  for (j in 1:n){
    resultat_innen[j] <- (y[i]-ybar)*(y[j]-ybar)
  }
...
for (i in 1:n){
  resultat_innen <- numeric(26)
  for (j in 1:n){
    resultat_innen[j] <- w[i,j] * (y[i]-ybar)*(y[j]-ybar)
  }
  resultat_aussen[i] <- sum(resultat_innen)
}
...
### Berechnung der Summe der Gewichte (`nenner2`)
...
Wir sind schon fast am Ziel! Versuche nun selbständig den Nenner aus dem zweiten Bruch zu berechnen.
