...
In a first step each criterion needs to be compared with another criteria in pairs. Use the following scale for weighting the criteria (see table @tbl-ahprating). 

```{r, echo = FALSE, purl = FALSE, message=F}
#| label: tbl-ahprating
#| tbl-cap: AHP Ratings
library(dplyr)
...
Create this matrix comparison matching your criteria, replacing the `0` values with your weights according to table @tbl-ahprating. Note that all diagonal values should equal to `1`.
...
![](images/ahp_random_index_by_saaty.png)
