...
tm_shape(wind250m) + tm_raster(style = "cont") 
```
...
```{r}
#| output: false
crs(strassen_classify, describe = TRUE)
```
...
#| echo: false
library(purrr)
list("wind_classify" = wind_classify, "neigung_classify" = neigung_classify) |>
  imap_dfr(~mutate(crs(.x, describe = TRUE), datensatz = .y, .before = 1))
```
...
crs(wind_classify) <- "EPSG:21781"
crs(neigung_classify) <- "EPSG:21781"
