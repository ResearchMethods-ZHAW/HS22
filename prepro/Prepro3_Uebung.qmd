...
Gegeben sei ein Datensatz "[sensors_combined.csv](https://moodle.zhaw.ch/mod/resource/view.php?id=602238)", mit den Temperaturwerten von drei verschiedenen Sensoren. Lade diesen Datensatz herunter,  importiere ihn als csv in R (als `sensors_combined`). 
...
Lade jetzt nochmal den Datensatz [weather.csv](https://moodle.zhaw.ch/mod/resource/view.php?id=602135) (Quelle MeteoSchweiz) herunter und importiere ihn als CSV mit den Korrekten Spaltentypen (`stn` als `factor`, `time` als `POSIXct`, `tre200h0` als `double`).

```{r}
weather <- read_csv("data/weather.csv", col_types = cols(col_factor(), col_datetime("%Y%m%d%H"), col_double()))
```
...
```{r}
#| fig.cap: baseplot mag keine long tables und macht aus den beiden Jahren eine kontinuierliche
#|   Linie
